...
var require_windows = __commonJS({
  "node_modules/isexe/windows.js"(exports2, module2) {
    ...
    function isexe(path20, options, cb) {
      fs10.stat(path20, function(er, stat) {
        cb(er, er ? false : checkStat(stat, path20, options));
      });
    }
    function sync(path20, options) {
      return checkStat(fs10.statSync(path20), path20, options);
    }
  }
});
...
var require_mode = __commonJS({
  "node_modules/isexe/mode.js"(exports2, module2) {
    ...
    function sync(path20, options) {
      return checkStat(fs10.statSync(path20), options);
    }
...
var require_main = __commonJS({
  "node_modules/vscode-jsonrpc/lib/node/main.js"(exports2) {
    ...
    function generateRandomPipeName() {
      ...
      let result;
      if (XDG_RUNTIME_DIR) {
        result = path20.join(XDG_RUNTIME_DIR, `vscode-ipc-${randomSuffix}.sock`);
      } else {
        result = path20.join(os5.tmpdir(), `vscode-${randomSuffix}.sock`);
      }
...
var import_coc46, import_fs7, import_path13, import_util, nulToken, updateImportsOnFileMoveName, UpdateImportsOnFileRenameHandler;
    ...
    UpdateImportsOnFileRenameHandler = class {
      ...
      async promptUser(newResources) {
        ...
        const response = await import_coc46.window.showInformationMessage(
          newResources.length === 1 ? `Update imports for '${import_path13.default.basename(newResources[0].fsPath)}'?` : this.getConfirmMessage(`Update imports for the following ${newResources.length} files?`, newResources),
          rejectItem,
          acceptItem
        );
        switch (response) {
      ...
      getConfirmMessage(start, resourcesToConfirm) {
        ...
        const paths = [start];
        paths.push("");
        paths.push(...resourcesToConfirm.slice(0, MAX_CONFIRM_FILES).map((r) => import_path13.default.basename(r.fsPath)));
...
var _ApplyRefactoringCommand = class _ApplyRefactoringCommand {
  ...
  async getTargetFile(document, file2, range) {
    ...
    return await import_coc23.window.showQuickPick(files, { title: "Move to file..." });
  }
};
...
var NodeLogDirectoryProvider = class {
  ...
  logDirectory() {
    try {
      const path20 = base;
      if (!fs6.existsSync(path20)) {
        fs6.mkdirSync(path20, { recursive: true });
      }
      return path20;
    } catch {
      return void 0;
...
var ResourceMap = class {
  ...
  isCaseInsensitivePath(path20) {
    if (isWindowsPath(path20)) {
      return true;
    }
    return path20[0] === "/" && this.onIsCaseInsenitiveFileSystem;
  }
...
function isWindowsPath(path20) {
  return /^[a-zA-Z]:\\/.test(path20);
}
...
var BufferSyncSupport = class {
  constructor(client, modeIds) {
    ...
    const pathNormalizer = (path20) => this.client.toPath(path20);
...
var ServiceConfigurationProvider = class {
  ...
  fixPathPrefixes(inspectValue) {
    const pathPrefixes = ["~" + import_path10.default.sep];
    for (const pathPrefix of pathPrefixes) {
      if (inspectValue.startsWith(pathPrefix)) {
        return import_path10.default.join(import_os3.default.homedir(), inspectValue.slice(pathPrefix.length));
      }
    }
    return inspectValue;
  }
...
var TypeScriptServerSpawner = class _TypeScriptServerSpawner {
  ...
  getTsServerArgs(kind, configuration, currentVersion, apiVersion, pluginManager, cancellationPipeName) {
    ...
    if (_TypeScriptServerSpawner.isLoggingEnabled(configuration)) {
      ...
      if (logDir) {
        tsServerLogFile = path14.join(logDir, `tsserver.log`);
...
var TypeScriptVersion = class {
  constructor(path20, _pathLabel) {
    this.path = path20;
    this._pathLabel = _pathLabel;
    this._api = null;
  }
  get tscPath() {
    return import_path11.default.resolve(this.path, "../bin/tsc");
  }
  get tsServerPath() {
    return import_path11.default.resolve(this.path, "tsserver.js");
  }
  get pathLabel() {
  ...
  getTypeScriptVersion(serverPath) {
    ...
    const p = serverPath.split(import_path11.default.sep);
    if (p.length <= 2) {
      return void 0;
    }
    const p2 = p.slice(0, -2);
    const modulePath = p2.join(import_path11.default.sep);
    let fileName = import_path11.default.join(modulePath, "package.json");
    if (!import_fs6.default.existsSync(fileName)) {
      if (import_path11.default.basename(modulePath) === "built") {
        fileName = import_path11.default.join(modulePath, "..", "package.json");
      }
    }
    if (!import_fs6.default.existsSync(fileName)) {
      return void 0;
...
var TypeScriptVersionProvider = class {
  ...
  get globalVersion() {
    ...
    if (!import_path11.default.isAbsolute(folder)) {
      folder = import_path11.default.join(import_coc37.workspace.root, folder);
    }
  ...
  getVersionFromTscPath(tscPath) {
    ...
    let libFolder = import_path11.default.resolve(tscPath, "../../lib");
  ...
  getLocalVersionFromFolder(workspaceRoot) {
    for (let folder of MODULE_FOLDERS) {
      let libFolder = import_path11.default.join(workspaceRoot, folder);
      if (import_fs6.default.existsSync(libFolder)) {
        let version = new TypeScriptVersion(libFolder);
        if (version.isValid) return version;
      }
  ...
  get bundledVersion() {
    try {
      ...
      const bundledVersion = new TypeScriptVersion(
        import_path11.default.dirname(file2),
        ""
      );
    ...
    for (const root of import_coc39.workspace.workspaceFolders || []) {
      ...
      for (const rootPrefix of rootPrefixes) {
        if (relativePath.startsWith(rootPrefix)) {
          return path16.join(import_coc39.Uri.parse(root.uri).fsPath, relativePath.replace(rootPrefix, ""));
        }
      }
    }
    return void 0;
...
var TypeScriptPluginPathsProvider = class {
  ...
  resolvePluginPath(pluginPath) {
    if (path17.isAbsolute(pluginPath)) {
      return [pluginPath];
    }
    ...
    return (import_coc40.workspace.workspaceFolders || []).map((workspaceFolder) => path17.join(import_coc40.Uri.parse(workspaceFolder.uri).fsPath, pluginPath));
  }
};
...
var TypeScriptServiceClient = class extends Disposable7 {
  constructor(context, modeIds, services2, tscPathVim) {
    ...
    this.pathSeparator = import_path12.default.sep;
    ...
    if (this._apiVersion.gte(API.v213)) {
      if (filepath.startsWith(this.inMemoryResourcePrefix + "untitled:")) {
        ...
        if (this.inMemoryResourcePrefix) {
          const dirName = import_path12.default.dirname(resource.path);
          const fileName = import_path12.default.basename(resource.path);
          if (fileName.startsWith(this.inMemoryResourcePrefix)) {
            resource = resource.with({ path: import_path12.default.posix.join(dirName, fileName.slice(this.inMemoryResourcePrefix.length)) });
          }
        }
        return resource.toString();
      }
  ...
  normalizedPath(resource) {
    ...
    switch (resource.scheme) {
      case file: {
        ...
        result = import_path12.default.normalize(result);
        ...
        if (this.inMemoryResourcePrefix) {
          ...
          if (fileName.startsWith(this.inMemoryResourcePrefix)) {
            resource = resource.with({
              path: import_path12.default.posix.join(
                dirName,
    ...
    switch (resource.scheme) {
      ...
      case memFs:
        for (const uri of uris.sort((a, b) => a.fsPath.length - b.fsPath.length)) {
          if (resource.fsPath.startsWith(uri.fsPath + import_path12.default.sep)) {
            return uri.fsPath;
          }
        }
